...
internal static class HitResultHelper
{
    ...
    public static Dictionary<HitResult, int> GenerateHitResultsForPredicted(IBeatmap beatmap, HitsResult hits, int mode)
    {
        switch (mode)
        {
            case 0:
                {
                    ...
                    return new Dictionary<HitResult, int>
                    {
                        { HitResult.Great, currentTotalHits == 0 ? 0 : hits.Hit300 * beatmap.HitObjects.Count / currentTotalHits },
                        { HitResult.Ok, currentTotalHits == 0 ? 0 : hits.Hit100 * beatmap.HitObjects.Count / currentTotalHits },
                        { HitResult.Meh, currentTotalHits == 0 ? 0 : hits.Hit50 * beatmap.HitObjects.Count / currentTotalHits },
                        { HitResult.Miss, currentTotalHits == 0 ? 0 : hits.HitMiss * beatmap.HitObjects.Count / currentTotalHits }
                    };
                }
            case 1:
                {
                    ...
                    return new Dictionary<HitResult, int>
                    {
                        { HitResult.Great, currentTotalHits == 0 ? 0 : hits.Hit300 * totalHits / currentTotalHits },
                        { HitResult.Ok, currentTotalHits == 0 ? 0 : hits.Hit100 * totalHits / currentTotalHits },
                        { HitResult.Miss, currentTotalHits == 0 ? 0 : hits.HitMiss * totalHits / currentTotalHits }
                    };
                }
            case 2:
                {
                    ...
                    return new Dictionary<HitResult, int>
                    {
                        { HitResult.Great, currentTotalHits == 0 ? 0 : hits.Hit300 * totalObjects / currentTotalHits },
                        { HitResult.LargeTickHit, currentTotalHits == 0 ? 0 : hits.Hit100 * totalObjects / currentTotalHits },
                        { HitResult.SmallTickHit, currentTotalHits == 0 ? 0 : hits.Hit50 * totalObjects / currentTotalHits },
                        { HitResult.SmallTickMiss, currentTotalHits == 0 ? 0 : hits.HitKatu * totalObjects / currentTotalHits },
                        { HitResult.Miss, currentTotalHits == 0 ? 0 : hits.HitMiss * totalObjects / currentTotalHits }
                    };
                }
            case 3:
                {
                    int currentTotalHits = hits.HitGeki + hits.Hit300 + hits.HitKatu + hits.Hit100 + hits.Hit50 + hits.HitMiss;
                    return new Dictionary<HitResult, int>
                    {
                        [HitResult.Perfect] = currentTotalHits == 0 ? 0 : hits.HitGeki * beatmap.HitObjects.Count / currentTotalHits,
                        [HitResult.Great] = currentTotalHits == 0 ? 0 : hits.Hit300 * beatmap.HitObjects.Count / currentTotalHits,
                        [HitResult.Good] = currentTotalHits == 0 ? 0 : hits.HitKatu * beatmap.HitObjects.Count / currentTotalHits,
                        [HitResult.Ok] = currentTotalHits == 0 ? 0 : hits.Hit100 * beatmap.HitObjects.Count / currentTotalHits,
                        [HitResult.Meh] = currentTotalHits == 0 ? 0 : hits.Hit50 * beatmap.HitObjects.Count / currentTotalHits,
                        [HitResult.Miss] = currentTotalHits == 0 ? 0 : hits.HitMiss * beatmap.HitObjects.Count / currentTotalHits
                    };
