...
namespace OsuMate.Services.Osu
{
    ...
    {
        public static OxyColor GetOxyColor(int judgement, ThemeSettings theme) => judgement switch
        {
        ...
        public static int GetJudgement(double offsetMs, Dictionary<HitResult, double> hitWindows)
        {
            ...
            if (perfect > 0 && abs <= perfect) return 1;
            if (great > 0 && abs <= great) return 2;
            if (good > 0 && abs <= good) return 3;
            if (ok > 0 && abs <= ok) return 4;
            if (meh > 0 && abs <= meh) return 5;
            return 6;
        }

        public static double GetWindow(Dictionary<HitResult, double> hitWindows, HitResult r)
            => hitWindows.TryGetValue(r, out var v) ? v : 0;

        public static double GetMaxWindow(Dictionary<HitResult, double> hitWindows)
            => hitWindows.Values.Where(x => x > 0).DefaultIfEmpty(0).Max();

        public static List<(int judgement, double msValue, double from, double to)> GetCenterLineSegments(
            Dictionary<HitResult, double> hitWindows)
        {
            ...
            {
                if (w > 0) result.Add((j, w, ToRatio(-w), ToRatio(w)));
            }
            return result;
        }
    }
