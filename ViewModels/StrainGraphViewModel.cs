...
namespace OsuMate.ViewModels
{
    ...
    {
        private OxyColor GetBlue() => ColorUtils.OxyFromHsl(216, _theme.PlotSaturation, _theme.PlotLightness);
        private OxyColor GetGreen() => ColorUtils.OxyFromHsl(144, _theme.PlotSaturation, _theme.PlotLightness);
        private OxyColor GetYellow() => ColorUtils.OxyFromHsl(72, _theme.PlotSaturation, _theme.PlotLightness);
        private OxyColor GetRed() => ColorUtils.OxyFromHsl(0, _theme.PlotSaturation, _theme.PlotLightness);
        private OxyColor GetPink() => ColorUtils.OxyFromHsl(288, _theme.PlotSaturation, _theme.PlotLightness);
        ...
        {
            _theme = theme;
            BlueBrush = new SolidColorBrush(ColorUtils.FromHsl(216, _theme.PlotSaturation, _theme.PlotLightness));
            GreenBrush = new SolidColorBrush(ColorUtils.FromHsl(144, _theme.PlotSaturation, _theme.PlotLightness));
            YellowBrush = new SolidColorBrush(ColorUtils.FromHsl(72, _theme.PlotSaturation, _theme.PlotLightness));
            RedBrush = new SolidColorBrush(ColorUtils.FromHsl(0, _theme.PlotSaturation, _theme.PlotLightness));
            PinkBrush = new SolidColorBrush(ColorUtils.FromHsl(288, _theme.PlotSaturation, _theme.PlotLightness));
        }
        ...
        private void RenderThemeOnly()
        {
            ...
            var skillLines = _plotModel.Series.OfType<LineSeries>().ToList();

            for (int i = 0; i < skillLines.Count - 1 && i < skillColors.Length; i++)
                skillLines[i].Color = skillColors[i];

            skillLines.LastOrDefault()!.Color = _theme.OxyTextColor;
        ...
        public void Update(int time)
        {
            ...
            if (_lastStrainTimeModified == 0) return;

            _currentTimeMs = Math.Max(0, time * _speedMultiplier - _firstObjectTimeModified);

            // Strainデータ（_values/_labels）自体はSetData()が呼ばれるまで変わらないため、
            // 既に描画済みなら縦線の位置だけ書き換えて全系列の再構築を避ける
            // （URTimeGraphViewModel.Updateと同様の間引き）
        ...
        public void Render()
        {
            ...
            {
                for (int j = 0; j < 5; j++)
                    if (GetValue(i, j) > maxValue) maxValue = GetValue(i, j);
            }
...
{
    (new LineSeries { Color = GetBlue(),   StrokeThickness = 2, MarkerType = MarkerType.None }, RhythmVisible),
    (new LineSeries { Color = GetGreen(),  StrokeThickness = 2, MarkerType = MarkerType.None }, ReadingVisible),
    (new LineSeries { Color = GetYellow(), StrokeThickness = 2, MarkerType = MarkerType.None }, ColourVisible),
    (new LineSeries { Color = GetRed(),    StrokeThickness = 2, MarkerType = MarkerType.None }, Stamina1Visible),
    (new LineSeries { Color = GetPink(),   StrokeThickness = 2, MarkerType = MarkerType.None }, Stamina2Visible),
};

            for (int i = 0; i < totalCount; i++)
            {
                double t = i * 400 * _speedMultiplier / 1000.0;
                for (int j = 0; j < 5; j++)
                    series[j].s.Points.Add(new DataPoint(t, GetValue(i, j)));
            }

            foreach (var (s, visible) in series)
                if (visible) model.Series.Add(s);

            // 現在位置の縦線（フィールドに保持し、Update()から縦線だけの軽量更新に使う）
        ...
        private string GetLabel(int index) =>
            _labels.Length > index ? _labels[index] : "-";

        private float GetValue(int index, int time)
        {
            if (_values.Count <= time) return 0;
