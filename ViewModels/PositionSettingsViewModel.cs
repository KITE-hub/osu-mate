...
{
    /// <summary>
    /// osu mate本体（App）と osu! 本体（Osu）の起動時ウィンドウ位置設定を管理するサブViewModel。
    ///
    /// リファクタリング分析レポート「段階的に進める」項目7に基づき、
    /// 元々 <see cref="SettingsViewModel"/> に混在していた「プリセット対象設定」のうち
    /// App/Osu 位置関連部分を切り出したもの（挙動は切り出し前と同一）。
    ///
    ...
    public sealed class PositionSettingsViewModel : ObservableBase
    {
        ...
        public bool AppPositionEnabled
        {
            ...
            set { _presetConfig().AppPositionEnabled = value; OnPropertyChanged(); _save(); }
        }
        public double AppX
        {
            get => _presetConfig().AppX;
            set { _presetConfig().AppX = value; OnPropertyChanged(); OnPropertyChanged(nameof(AppPositionText)); _save(); }
        }
        public double AppY
        {
            get => _presetConfig().AppY;
            set { _presetConfig().AppY = value; OnPropertyChanged(); OnPropertyChanged(nameof(AppPositionText)); _save(); }
        }
        ...
        public bool OsuPositionEnabled
        {
            ...
            set { _presetConfig().OsuPositionEnabled = value; OnPropertyChanged(); _save(); }
        }
        public double OsuX
        {
            get => _presetConfig().OsuX;
            set { _presetConfig().OsuX = value; OnPropertyChanged(); OnPropertyChanged(nameof(OsuPositionText)); _save(); }
        }
        public double OsuY
        {
            get => _presetConfig().OsuY;
            set { _presetConfig().OsuY = value; OnPropertyChanged(); OnPropertyChanged(nameof(OsuPositionText)); _save(); }
        }
